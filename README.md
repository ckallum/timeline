...
viewer/         local web viewer (React + Tailwind + Vite)
...
cd viewer && npm run build && npm run preview   # http://localhost:4173
```

The viewer itself is a React + Tailwind app in `viewer/`. Data comes from `viewer/data/timeline.json`, generated by `scripts/build-timeline-data.ts` (auto-rebuilt on each Claude session Stop hook).
